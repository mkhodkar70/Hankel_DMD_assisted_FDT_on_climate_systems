...
R_true_u = uXTave(end/2+1:end, :);
R_true_T = TXTave(end/2+1:end, :);


if(forcing_type == 1)
    text = ['Test_cases/Response', num2str(forcing_type), 'n.mat'];
    load(text, 'uXTave', 'TXTave');
    R_true_u = 0.5*(R_true_u - uXTave(end/2+1:end, :));
    R_true_T = 0.5*(R_true_T - TXTave(end/2+1:end, :));
elseif(forcing_type == 2)
    text = 'Test_cases/CTL.mat';
    load(text, 'uXTave', 'TXTave');
    R_true_u = R_true_u - uXTave(end/2+1:end, :);
    R_true_T = R_true_T - TXTave(end/2+1:end, :);
end
...
set(gca, 'Fontsize', 12)
R_fdt_T = CT*R_true_T + (1-CT)*R_fdt_T;
...
colormap(b2r(-0.2, 0.2));
set(gca, 'Fontsize', 12)
R_fdt_u = Cu*R_true_u + (1-Cu)*R_fdt_u;
xlabel('$\mu$', 'interpreter', 'latex');
ylabel('$p \ (hPa)$', 'interpreter', 'latex');
